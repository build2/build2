...
// NOTE: don't forget to update signal handling in
//       libbutl/tests/process-group/driver.cxx if changing anything here.
//
#ifndef _WIN32
// Handle a terminating signal (SIGTERM, etc) by performing cleanups,
// forwarding it to the active process groups, and terminating with the
// specified signal. Assume that the default handler has been installed, so
// that this signal can be re-raised to terminate the program. If the unblock
// argument is true, then unblock the signal prior to raising it.
//
// If this signal is caused by external conditions (raised by the user or an
// external program), then forward it to the running process groups, as if we
// are all members of the same group. Otherwise, if it is likely caused by
// internal problems (SIGABRT, etc), then just kill the active groups.
//
...
// active process groups and then performing the default action.
//
// Note: for parallel execution the process spawn mutex is expected to be
//       acquired to protect the process groups list.
//
...
job_control_signal_handler (int sig)
{
  ...
  //
  // - We will probably have to deal with the "parent stuck in posix_spawn on
  //   SIGTSTP" race condition (see the process constructor for details),
  //   which is quite easy to reproduce.
  //
  ...
  // process suspension. If we are running in parallel, SIGCONT will stay
  // pending when we leave raise() and will be returned by some subsequent
  // sigwait() call. As we keep the mutex acquired since handling SIGTSTP (see
  // sigwait_thread_function() for details), the process group list may not
  // change by the time we handle SIGCONT.
  //
  if (sig == SIGTSTP && raise (SIGSTOP) != 0)
    _exit (1);
}

// Signal handler function for serial execution.
//
extern "C" void
build2_b_signal_handler (int sig)
{
  // Note: memset(), sigemptyset(), sigaction(), _exit(), and
  //       *_signal_handler() are all async signal-safe.
  //
  switch (sig)
  {
  case SIGINT:
  ...
  case SIGABRT:
    {
      // Restore the default signal handler so that the signal can be raised,
      // exiting if anything goes sideways.
      //
      // Note that while we handle this signal, it stays temporarily blocked
      // by the kernel for the time of the handler execution. Thus,
      // terminating_signal_handler() will only make the signal pending.
      //
      struct sigaction sa;
...
// Signal handling thread function for parallel execution.
//
...
sigwait_thread_function (void* d)
{
  ...
  // is raised and, afterwards, sigwait() will return SIGTERM first.
  //
    ...
    case SIGABRT:
      {
        ...
        // signal to.
        //
        // Note that we don't track the mutex locking anymore, since
        // terminating_signal_handler() won't return.
        //
        if (!locked)
          process::mutex.lock_shared ();

        terminating_signal_handler (sig, true /* unblock */); // No return.
        assert (false);
        break;
      }
    case SIGTSTP:
    case SIGCONT:
      {
        ...
        // groups. Only release the mutex after SIGCONT is handled, when this
        // process and all the suspended process groups are resumed, to make
        // sure that no new groups are spawned in between and we don't forward
        // SIGCONT to non-suspended process groups.
        //
        // Note, however, that SIGCONT may well arrive out of order, for
        // example, to a program which was not suspended. Thus, we always lock
        // an unlocked mutex here.
        //
        if (!locked)
        {
          process::mutex.lock_shared ();
