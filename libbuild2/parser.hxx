...
namespace build2
{
  ...
  protected:
    ...
    // the save and stop modes, potentially peeks the next 2 characters from
    // the stream using the lexer. Supports replays, saving the result in the
    // token's keyword member in the save mode and using it later, in the play
    // mode.
    //
    bool
    keyword (const token&);
  ...
  protected:
    ...
    // sequence and behave as if we go via replay_stop(), but keeping the
    // collected tokens.
    //
    void
    ...
    struct replay_guard
    {
      ...
      //   saving in the save mode and switch to saving from the current
      //   replay position (assumed to be the end of the sequence) in the play
      //   mode.
      //
      void
