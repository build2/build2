      ...
      if (tt == type::colon)
      {
                                            ...
                                            const location& nloc)
        {
          ...
          if (!n.dir.empty ())
          {
            ...
            // Stash the scope the target pattern is defined in.
            //
            ...
            // that the pattern directory is outside of the output directory,
            // in which case we fail. But let's also help the user a bit, who
            // often thinks that the following target patterns are the same:
            //
            // file{$src_base/bar/*}
            // file{bar/*}
            //
            ...
            if (rs == nullptr)
            {
              assert (pbase_ != nullptr); // Refers to the pattern directory.

              diag_record dr (fail (nloc));
              dr << "target pattern directory " << pbase_->representation ()
                 << " is outside of output directory";

              // If the specified pattern directory belongs to the project
              // source directory, then print its path as relative to the
              // stashed scope source directory.
              //
              const scope* rs (s->root_scope ());

              if (rs != nullptr && pbase_->sub (rs->src_path ()))
              {
                dr << info << "perhaps you meant pattern directory "
