...
namespace build2
{
  namespace test
  {
    namespace script
    {
      ...
      void parser::
      pre_parse_directive (token& t, type& tt, lines* ls)
      {
        ...
        if (d == "include")
          pre_parse_include_line (move (args), ls, move (l));
        else
          assert (false); // Unhandled directive.
      }

      // If ls is not NULL, then pre-parse the specified files into the
      // the test scope (ls). Otherwise, pre-parse them into the current
      // group scope (group_).
      //
      void parser::
      pre_parse_include_line (names args, lines* ls, location dl)
      {
        auto i (args.begin ());
        ...
        // Process arguments.
        //
        auto include = [&dl, once, ls, this] (string n) // throw invalid_path
        {
          ...
          if (include_set_->insert (p).second || !once)
          {
            try
            {
              ...
              token t (
                ls == nullptr
                ? pre_parse_group_body ()
                : pre_parse_command_lines (*ls, nullopt /* block_type */));
