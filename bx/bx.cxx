...
namespace build2
{
  ...
  static int
  run_script (bx_options&, path, strings args);
  ...
  try
  {
    ...
    for (bool opt (true) /*, var (false)*/; scan.more (); )
    {
      if (opt)
      {
        ...
        // this task.
        //
        if (strcmp (scan.peek (), "--") == 0 && !r.script && !r.task)
        {
          scan.next ();
    ...
    try
    {
      ...
      //
      // Note: the default arguments are only allowed for the task runner.
      //
      ...
      //    overrides if BUILD2_VAR_OVR is already set, which can be a good
      //    thing for some situations and not so good for others:
      //
      //    b (sets BUILD2_VAR_OVR) -> bx (loads ovrs) -> b
      //    bx (loads ovrs) -> b (sets BUILD2_VAR_OVR) ->bx (loads ovrs) -> b
      //
      //    This is good for the first case. For the second case it
      //    loads/prepends overrides twice, which is not ideal but is probably
      //    harmless.
      //
      if (r.task)
        r.cmd_vars =
          merge_default_arguments (
            def_ops,
            r.cmd_vars,
            [] (const default_options_entry<b_options>& e, const strings&)
            {
              path_name fn (e.file);
              ...
              for (const string& a: e.arguments)
                //
                // @@ Probably, makes sense to turn the lambda
...
main (int argc, char* argv[])
{
  ...
  try
  {
    ...
    if (cmdl.script)
      return run_script (ops, move (*cmdl.script), move (cmdl.args));
    else
      return run_task (ops, move (*cmdl.task), move (cmdl.args));
  }
  catch (const failed&)
  {
